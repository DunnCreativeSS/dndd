...
rvec shows total number of DNDL satoshis created since genesis for:
...
- Missed blocks not modeled (lowers DNDL supply, small)
- Miner queue length very approximately modeled (assumed to go to 100 during the first blocks and then stay there) (may lower or raise DNDL supply, very small)
- Creation / destruction of DNDL used to back DNDL2 not modeled (moves DNDL supply in direction opposite to changes in dollar value of 1 DNDL, large)
- Interest paid to DNDL2 not modeled (raises DNDL supply, medium)
- Lost / forgotten private keys / wallets and deliberate burning of DNDL not modeled (lowers DNDL supply, unknown but likely small)
...
/*
#define DNDLIT_GENESIS_TIME                    (fc::time_point_sec(1458835200))
#define DNDLIT_MINING_TIME                     (fc::time_point_sec(1458838800))
#define DNDLIT_FIRST_CASHOUT_TIME              (fc::time_point_sec(1467590400))  /// July 4th
*/
   ...
   {
      uint32_t vesting_factor = (block_num < DNDLIT_START_VESTING_BLOCK) ? 0 : 9;

      share_type curate_reward   = calc_percent_reward_per_block< DNDLIT_CURATE_APR_PERCENT >( current_supply );
      reward_delta[ CURATE_OFF ] = std::max( curate_reward, DNDLIT_MIN_CURATE_REWARD.amount );
      reward_delta[ VCURATE_OFF ] = reward_delta[ CURATE_OFF ] * vesting_factor;

      share_type content_reward  = calc_percent_reward_per_block< DNDLIT_CONTENT_APR_PERCENT >( current_supply );
      reward_delta[ CONTENT_OFF ] = std::max( content_reward, DNDLIT_MIN_CONTENT_REWARD.amount );
      reward_delta[ VCONTENT_OFF ] = reward_delta[ CONTENT_OFF ] * vesting_factor;

      share_type producer_reward = calc_percent_reward_per_block< DNDLIT_PRODUCER_APR_PERCENT >( current_supply );
      reward_delta[ PRODUCER_OFF ] = std::max( producer_reward, DNDLIT_MIN_PRODUCER_REWARD.amount );
      ...
      if( (block_num % DNDLIT_MAX_WITNESSES) == 0 )
         ++pow_deficit;

      if( pow_deficit > 0 )
      {
         pow_reward = calc_percent_reward_per_round< DNDLIT_POW_APR_PERCENT >( current_supply );
         pow_reward = std::max( pow_reward, DNDLIT_MIN_POW_REWARD.amount );
         if( block_num < DNDLIT_START_MINER_VOTING_BLOCK )
            pow_reward *= DNDLIT_MAX_WITNESSES;
         --pow_deficit;
      }
      ...
      if( (block_num > liquidity_begin_block) && ((block_num % DNDLIT_LIQUIDITY_REWARD_BLOCKS) == 0) )
      {
         liquidity_reward = calc_percent_reward_per_hour< DNDLIT_LIQUIDITY_APR_PERCENT >( current_supply );
         liquidity_reward = std::max( liquidity_reward, DNDLIT_MIN_LIQUIDITY_REWARD.amount );
      }
   ...
   for( uint32_t b=1; b<10*DNDLIT_BLOCKS_PER_YEAR; b++ )
   {
